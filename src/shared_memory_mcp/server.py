...
def search_notes(query:str='',tag:str='',limit:int=5,before:str='',since:str='',parent_id:str='')->dict:
    ...
    return request('/notes?'+urllib.parse.urlencode({k:v for k,v in {'q':query,'tag':tag,'limit':min(50,max(1,limit)),'before':before,'since':since,'parent_id':parent_id}.items() if v!=''}))
@mcp.tool(annotations=ToolAnnotations(readOnlyHint=True,openWorldHint=True))
